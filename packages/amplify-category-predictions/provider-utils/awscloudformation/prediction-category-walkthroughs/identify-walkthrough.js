...
async function addS3ForIdentity(context, storageAccess, bucketName, predictionsResourceName) {
  ...
  const { getAllS3Defaults, getAllAuthDefaults, getAllAuthAndGuestDefaults, generatePolicyPrefix} = require(defaultValuesSrc);
  const projectBackendDirPath = context.amplify.pathManager.getBackendDirPath();
  ...
  answers.policyUUID= generatePolicyPrefix(); //generate the policy prefix for autogenerated policy names.
  if (answers.storageAccess === 'authAndGuest') {
    Object.assign(answers, getAllAuthAndGuestDefaults(answers.policyUUID));
    allowUnauthenticatedIdentities = true;
  } else {
    Object.assign(answers, getAllAuthDefaults(answers.policyUUID));
  }
  const s3Resource = {
